...
def resamplingby_searchsorted(X1,W1):
    Wcum = np.cumsum(W1)
    print("W1: ", W1, "len: ",len(W1))
    print("Wcum: ", Wcum)
    I = np.searchsorted(Wcum,np.random.rand(len(W1))).tolist()
    print("I: ", I)
    X1 = X1[I]
    return(X1,W1)
...
def montecarlo(X0,W0, obs):
    ...
    # input()
    Wcum = sum(W1)
    W1 = [W1[i]/Wcum for i in range(0,len(W1))]

    #RESAMPLING METHODS

    #[X1,W1] = resamplingby_searchsorted(X1,W1)  # Resampling using searchsorted
    [X1,W1] = resampling_algorithm_from_article(X1,W1) # Resampling using the CDF
    #X1 = np.random.choice(a = X1,p = W1, replace = True,size = len(X1)) # Resampling but we have no indicies
    return(X1,W1)
